...
export function StageProgressBar({ currentStage, status }: Props) {
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors ${
                      ...
                      ? isConvicted
                        ...
                        : 'bg-nyaya-navy border-nyaya-navy text-white'
                      : active
                      ? 'bg-nyaya-saffron border-nyaya-saffron text-white animate-pulse'
              ...
              {i < STAGES.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-1 ${
                    i < currentIdx ? 'bg-nyaya-navy' : 'bg-gray-200'
                  }`}
                />
              )}
            </div>
